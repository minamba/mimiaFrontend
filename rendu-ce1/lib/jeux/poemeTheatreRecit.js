...
exports.bilan = exports.PHRASES = exports.MANCHES = exports.GENRES = exports.EXTRAITS = void 0;
exports.serie = serie;
...
const EXTRAITS = exports.EXTRAITS = [['Dans le jardin, la pluie\nFait chanter les feuilles ;\nEt la terre qui s’ennuie\nBoit tout ce qu’elle cueille.', 'poeme'], ['Petit escargot du soir,\nTu portes ta maison ;\nTu ne vas pas bien loin, mais voir\nLe monde est ta leçon.', 'poeme'], ['La mer a mis sa robe grise,\nLe ciel a fermé ses volets ;\nSur la plage, un vent qui frise\nEmporte les derniers galets.', 'poeme'], ['Mon crayon court sur la page,\nIl trace un chemin, un nuage,\nUne maison, un bateau,\nEt le soleil tout là-haut.', 'poeme'], ['LUCIE (en chuchotant). — Tu as entendu ce bruit ?\nMARC. — Quel bruit ? Je n’entends rien.\nLUCIE. — Là, derrière la porte !', 'theatre'], ['LE ROI (furieux). — Qui a mangé mes confitures ?\nLE VALET (tremblant). — Ce n’est pas moi, Majesté !\nLE ROI. — Alors qui ?', 'theatre'], ['LA MÈRE. — Range ta chambre, s’il te plaît.\nTHÉO (soupirant). — Encore ? Je l’ai rangée hier !\nLA MÈRE. — Hier, c’était hier.', 'theatre'], ['LE LOUP (à part). — Quel joli petit agneau…\nL’AGNEAU. — Bonjour, monsieur le loup !\nLE LOUP (souriant). — Bonjour, mon ami.', 'theatre'], ['Ce matin-là, Nina se leva avant tout le monde. Elle enfila ses bottes, prit son panier et partit vers la forêt. Le brouillard couvrait encore les champs.', 'recit'], ['Il était une fois un meunier qui n’avait qu’un âne et un chat. Quand il mourut, ses fils se partagèrent ce maigre héritage.', 'recit'], ['Le bateau quitta le port à l’aube. Pendant trois jours, les marins ne virent que la mer. Le quatrième jour, enfin, une île apparut.', 'recit'], ['Quand Samir ouvrit la boîte, il n’en crut pas ses yeux. À l’intérieur, un petit dragon dormait, roulé en boule.', 'recit']];
function serie(graine = Date.now()) {
  ...
  return melanger(tires).map(([question, genre]) => ({
    ...
    question,
    bonne: genre,
    choix: Object.entries(GENRES).map(([cle, libelle]) => ({
      cle,
      libelle
    }))
  }));
