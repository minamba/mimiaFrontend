...
function serie(graine = Date.now()) {
  ...
  for (let i = 0; i < 4; i += 1) {
    ...
    const d = dixiemes[i];
    const bonne = (0, _fractionsCM.virgule)(net(debut + d / 10));
    ...
    liste.push({
      ...
      dixiemes: d,
      bonne,
      choix: melanger([bonne, zero, colle]).map(c => ({
        cle: c,
        libelle: c
      })),
      pieges: {
  ...
  for (let i = 0; i < 4; i += 1) {
    ...
    const u = entre(1, 9);
    const d = entre(2, 9);
    const a = (0, _fractionsCM.virgule)(net(u + d / 10));
    const b = (0, _fractionsCM.virgule)(net(u + (d - 1) / 10 + entre(1, 9) / 100));
    liste.push({
      consigne: 'plus-grand',
      question: `${a}   ou   ${b} ?`,
      bonne: a,
      choix: melanger([a, b]).map(c => ({
        cle: c,
        libelle: c
      })),
      pieges: {
