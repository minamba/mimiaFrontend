...
const INGREDIENTS = [['farine', 'g', [200, 240, 300, 400]], ['lait', 'cL', [40, 60, 80]], ['œufs', '', [2, 4, 6]], ['sucre', 'g', [100, 120, 160]], ['beurre', 'g', [80, 100, 120]]];

/** [situation, oui ou non]. */
const SITUATIONS = exports.SITUATIONS = [['3 cahiers coûtent 6 €. 6 cahiers coûtent 12 €.', true], ['À 5 ans, Léo mesure 1,10 m. À 10 ans, il mesure 1,40 m.', false], ['Une voiture fait 100 km en 1 heure, et 200 km en 2 heures, toujours à la même vitesse.', true], ['Un bébé de 1 mois pèse 4 kg. À 2 mois, il pèse 5 kg.', false], ['1 kg de pommes coûte 3 €. 2 kg coûtent 6 €.', true], ['À 8 ans, Zoé chausse du 31. À 16 ans, elle chausse du 39.', false]];
...
function serie(graine = Date.now()) {
  ...
  melanger(INGREDIENTS).concat(melanger(INGREDIENTS)).slice(0, 6).forEach(([nom, unite, quantites], i) => {
    ...
    liste.push({
      ...
      bonne,
      choix: melanger((0, _outils.troisChoix)(bonne, [ajout, double], k => bonne + k * (pour4 / 4))).map(c => ({
        cle: c,
        libelle: avec(c, unite, nom)
      })),
      // L'ajout d'abord nommé : s'il tombe sur le double, c'est lui qu'on explique.
      pieges: {
        [double]: 'recette-regle',
        [ajout]: 'ajout'
      }
    });
  });
  melanger(SITUATIONS.filter(([, p]) => p)).slice(0, 1).concat(melanger(SITUATIONS.filter(([, p]) => !p)).slice(0, 1)).forEach(([question, oui]) => {
    liste.push({
      ...
      choix: [{
        cle: 'oui',
        libelle: 'Oui, c’est proportionnel'
      }, {
        cle: 'non',
        libelle: 'Non'
      }]
...
function verdict(m, cle) {
  ...
  return (_m$pieges$cle = m.pieges[cle]) !== null && _m$pieges$cle !== void 0 ? _m$pieges$cle : 'recette-regle';
}
