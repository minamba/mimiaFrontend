...
exports.bilan = exports.SYNONYMES = exports.SUFFIXES = exports.PREFIXES = exports.PHRASES = exports.MANCHES = void 0;
exports.serie = serie;
...
const PREFIXES = exports.PREFIXES = [['possible', 'impossible', 'inpossible'], ['poli', 'impoli', 'inpoli'], ['patient', 'impatient', 'inpatient'], ['buvable', 'imbuvable', 'inbuvable'], ['lisible', 'illisible', 'inlisible'], ['légal', 'illégal', 'inlégal'], ['régulier', 'irrégulier', 'inrégulier'], ['réel', 'irréel', 'inréel'], ['visible', 'invisible', 'imvisible'], ['connu', 'inconnu', 'imconnu']];

/** [adjectif, adverbe, adverbe tiré du masculin, nom de la famille]. */
const SUFFIXES = exports.SUFFIXES = [['lent', 'lentement', 'lentment', 'lenteur'], ['doux', 'doucement', 'douxment', 'douceur'], ['heureux', 'heureusement', 'heureuxment', 'bonheur'], ['joyeux', 'joyeusement', 'joyeuxment', 'joie'], ['léger', 'légèrement', 'légerment', 'légèreté'], ['fier', 'fièrement', 'fierment', 'fierté'], ['sérieux', 'sérieusement', 'sérieuxment', 'sérieux']];

/** [mot, synonyme, contraire, même famille]. */
const SYNONYMES = exports.SYNONYMES = [['content', 'joyeux', 'triste', 'contentement'], ['grand', 'immense', 'petit', 'grandir'], ['commencer', 'débuter', 'finir', 'commencement'], ['crier', 'hurler', 'chuchoter', 'cri'], ['beau', 'joli', 'laid', 'beauté'], ['peur', 'frayeur', 'courage', 'peureux'], ['rapide', 'vif', 'lent', 'rapidement'], ['triste', 'malheureux', 'gai', 'tristesse']];
function serie(graine = Date.now()) {
  ...
  melanger(PREFIXES).slice(0, 3).forEach(([mot, bonne, faux]) => {
    ...
    liste.push({
      ...
      bonne,
      choix: melanger([bonne, faux, leurre]).map(c => ({
        cle: c,
        libelle: c
      })),
      pieges: {
        [faux]: 'prefixe-lettre',
        [leurre]: 'prefixe-autre'
      }
    });
  });
  ...
  melanger(SUFFIXES.filter(s => s[0] !== s[3])).slice(0, 2).forEach(([mot, bonne, faux, nom]) => {
    liste.push({
      consigne: 'suffixe',
      question: `« ${mot} » → un mot en -ment`,
      bonne,
      choix: melanger([bonne, faux, nom]).map(c => ({
        cle: c,
        libelle: c
      })),
      pieges: {
        [faux]: 'suffixe-feminin',
        [nom]: 'suffixe-nom'
      }
    });
  });
  melanger(SYNONYMES).slice(0, 3).forEach(([mot, bonne, contraire, famille]) => {
    liste.push({
      consigne: 'synonyme',
      question: `Un synonyme de « ${mot} »`,
      bonne,
      choix: melanger([bonne, contraire, famille]).map(c => ({
        cle: c,
        libelle: c
      })),
      pieges: {
        [contraire]: 'synonyme-contraire',
        [famille]: 'synonyme-famille'
      }
    });
  });
