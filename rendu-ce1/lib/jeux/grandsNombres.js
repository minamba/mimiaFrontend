...
exports.enChiffres = exports.bilan = exports.PHRASES = exports.MANCHES = void 0;
exports.serie = serie;
...
const enChiffres = n => String(n).replace(/\B(?=(\d{3})+(?!\d))/g, ' ');

/** Un zéro déplacé dans la classe des mille : 045 → 405. */
exports.enChiffres = enChiffres;
...
function serie(graine = Date.now()) {
  ...
  for (let i = 0; i < MANCHES; i += 1) {
    ...
    const nombres = (0, _outils.troisChoix)(n, pieges, k => n + k * 1000);
    const lire = i % 2 === 0;
    liste.push({
      nombre: n,
      consigne: lire ? 'lire' : 'ecrire',
      question: lire ? enChiffres(n) : (0, _nombresEnLettres.enLettres)(n),
      bonne: n,
      choix: melanger(nombres).map(x => ({
        cle: x,
        libelle: lire ? (0, _nombresEnLettres.enLettres)(x) : enChiffres(x)
      }))
    });
  }
  return liste;
