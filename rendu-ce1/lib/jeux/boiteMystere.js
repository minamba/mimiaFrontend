...
function serie(graine = Date.now()) {
  ...
  for (let i = 0; i < 3; i += 1) {
    ...
    liste.push({
      ...
      bonne: x,
      choix: melanger((0, _outils.troisChoix)(x, [s + a], k => x + 10 * k)).map(c => ({
        cle: c,
        libelle: String(c)
      })),
      pieges: {
  ...
  for (let i = 0; i < 3; i += 1) {
    ...
    liste.push({
      ...
      question: `${BOITE} × ${a} = ${p}`,
      bonne: x,
      choix: melanger((0, _outils.troisChoix)(x, [p - a, p + a], k => x + k)).map(c => ({
        cle: c,
        libelle: String(c)
      })),
      pieges: {
        [p - a]: 'multiplication-moins',
        [p + a]: 'multiplication-plus'
      }
    });
  }
  for (let i = 0; i < 2; i += 1) {
    ...
    liste.push({
      ...
      bonne: x,
      choix: melanger((0, _outils.troisChoix)(x, [...pieges, r - a], k => x + k)).map(c => ({
        cle: c,
        libelle: String(c)
      })),
      pieges: {
        ...(pieges.length ? {
          [desordre]: 'pense-ordre'
        } : {}),
        [r - a]: 'pense-moitie'
      }
    });
