...
exports.bilan = exports.POLYSEMIE = exports.PHRASES = exports.NIVEAUX = exports.MANCHES = exports.CONTRAIRES = void 0;
exports.serie = serie;
...
const POLYSEMIE = exports.POLYSEMIE = [['Clique avec la souris de l’ordinateur.', 'souris', 'un petit appareil qu’on tient dans la main', 'un petit animal gris', 'un morceau de fromage'], ['Prends une feuille et un crayon.', 'feuille', 'un morceau de papier', 'la partie verte d’un arbre', 'une page de journal plié'], ['Il a un bouton sur le nez.', 'bouton', 'un petit gonflement de la peau', 'ce qui ferme une chemise', 'une touche qu’on presse'], ['Regarde-toi dans la glace.', 'glace', 'un miroir', 'un dessert froid', 'de l’eau gelée'], ['Le vol pour Paris part à midi.', 'vol', 'un voyage en avion', 'le fait de voler un objet', 'le mouvement d’un oiseau'], ['Le moteur de la voiture tourne.', 'tourner', 'fonctionner', 'changer de direction', 'faire un tour sur soi-même']];

/** [mot, niveau]. */
const NIVEAUX = exports.NIVEAUX = [['une bagnole', 'familier'], ['un bouquin', 'familier'], ['la frousse', 'familier'], ['le boulot', 'familier'], ['une voiture', 'courant'], ['un livre', 'courant'], ['la peur', 'courant'], ['le travail', 'courant'], ['une demeure', 'soutenu'], ['un ouvrage', 'soutenu'], ['l’effroi', 'soutenu'], ['un labeur', 'soutenu']];

/** [mot, contraire, synonyme, même famille]. */
const CONTRAIRES = exports.CONTRAIRES = [['généreux', 'avare', 'gentil', 'générosité'], ['courageux', 'peureux', 'brave', 'courage'], ['ancien', 'moderne', 'vieux', 'anciennement'], ['accepter', 'refuser', 'admettre', 'acceptation'], ['rapide', 'lent', 'vif', 'rapidité'], ['calme', 'agité', 'tranquille', 'calmement']];
function serie(graine = Date.now()) {
  ...
  melanger(POLYSEMIE).slice(0, 3).forEach(([phrase, mot, bonne, premier, autre]) => {
    liste.push({
      ...
      bonne,
      choix: melanger([bonne, premier, autre]).map(c => ({
        cle: c,
        libelle: c
      }))
    });
  ...
  niveaux.forEach(n => {
    ...
    liste.push({
      ...
      bonne: n,
      choix: ['familier', 'courant', 'soutenu'].map(c => ({
        cle: c,
        libelle: c
      }))
    });
  ...
  melanger(CONTRAIRES).slice(0, 2).forEach(([mot, bonne, synonyme, famille]) => {
    liste.push({
      ...
      bonne,
      choix: melanger([bonne, synonyme, famille]).map(c => ({
        cle: c,
        libelle: c
      })),
      pieges: {
        [synonyme]: 'contraire-synonyme',
        [famille]: 'contraire-famille'
      }
    });
  });
