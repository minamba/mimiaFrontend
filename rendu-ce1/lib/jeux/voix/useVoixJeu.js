...
exports.adresseReplique = adresseReplique;
exports.default = useVoixJeu;
var _react = require("react");
var _repliques = require("./repliques");
/**
...
function adresseReplique(professeur, cle) {
...
function charger(ctx, url) {
  if (!tampons.has(url)) {
    ...
    .then(octets => new Promise((ok, ko) => {
      ...
      return {
        tampon,
        debut: a / tampon.sampleRate,
        duree: (b - a) / tampon.sampleRate
      };
    });
...
function useVoixJeu(matiereCode) {
  var _PROFESSEUR_PAR_MATIE;
  const professeur = (_PROFESSEUR_PAR_MATIE = _repliques.PROFESSEUR_PAR_MATIERE[matiereCode]) !== null && _PROFESSEUR_PAR_MATIE !== void 0 ? _PROFESSEUR_PAR_MATIE : null;
  const [muet, setMuet] = (0, _react.useState)(lireMuet);
  ...
  const [enCours, setEnCours] = (0, _react.useState)(null);
  ...
  const muetRef = (0, _react.useRef)(muet);
  muetRef.current = muet;
  const arreter = (0, _react.useCallback)(() => {
    jeton.current += 1;
    setEnCours(null);
    const {
      lecteur,
  ...
  const jouerParLecteurs = (0, _react.useCallback)((cles, mien) => {
    const suivante = i => {
      ...
      setEnCours(cles[i]);
      const apres = () => {
        if (i + 1 >= cles.length) suivante(i + 1);else enVol.current.minuteries.push(setTimeout(() => suivante(i + 1), SILENCE_ENTRE * 1000));
      };
      audio.addEventListener('ended', apres, {
        once: true
      });
      audio.addEventListener('error', apres, {
        once: true
      });
      try {
  ...
  const jouerParMoteur = (0, _react.useCallback)(async (ctx, cles, mien) => {
    try {
      if (ctx.state === 'suspended') await ctx.resume();
      // Un son manquant est sauté, pas la suite entière : les autres phrases
      // gardent l'enchaînement fluide du moteur.
      const sons = await Promise.all(cles.map(c => charger(ctx, adresseReplique(professeur, c)).catch(() => null)));
      if (jeton.current !== mien) return;
  ...
  const jouer = (0, _react.useCallback)(repliques => {
    arreter();
    const cles = (Array.isArray(repliques) ? repliques : [repliques]).map(r => typeof r === 'string' ? r : r === null || r === void 0 ? void 0 : r.cle).filter(Boolean);
    if (!professeur || cles.length === 0) return;
    const mien = jeton.current;
    const ctx = moteur();
    if (ctx && typeof fetch === 'function') jouerParMoteur(ctx, cles, mien);else jouerParLecteurs(cles, mien);
  }, [arreter, professeur, jouerParMoteur, jouerParLecteurs]);

  /** Dire une phrase — sauf si l'enfant a coupé la voix. */
  const dire = (0, _react.useCallback)(replique => {
    if (muetRef.current) {
      arreter();
      return;
    }
   ...
   * un silence.
   */
  const reecouter = (0, _react.useCallback)(replique => {
    jouer(replique);
  }, [jouer]);
  const basculerMuet = (0, _react.useCallback)(() => {
  ...
  (0, _react.useEffect)(() => arreter, [arreter]);
  return {
    dire,
    reecouter,
    muet,
