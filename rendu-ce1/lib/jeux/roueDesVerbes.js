...
exports.VERBES = exports.TEMPS = exports.PRONOMS = exports.PHRASES = exports.MANCHES = void 0;
exports.avecPronom = avecPronom;
exports.bilan = bilan;
exports.forme = forme;
exports.formesCE2 = formesCE2;
exports.formesProposees = formesProposees;
exports.regleCE2 = regleCE2;
exports.serie = serie;
...
const PRONOMS = exports.PRONOMS = ['je', 'tu', 'il', 'elle', 'nous', 'vous', 'ils', 'elles'];
...
const VERBES = exports.VERBES = ['être', 'avoir', 'chanter', 'jouer', 'aimer', 'parler', 'danser', 'regarder', 'marcher', 'écouter'];
const FINS_ER = ['e', 'es', 'e', 'ons', 'ez', 'ent'];

/** La forme d'un verbe à une personne : « chantons ». */
function forme(verbe, pronom, temps = 'present') {
 ...
 * temps.
 */
function formesCE2(verbe, pronom, temps, tirer) {
  var _sonnePareil$;
  const bonne = forme(verbe, pronom, temps);
  const memeTemps = [...new Set(['je', 'tu', 'il', 'nous', 'vous', 'ils'].map(x => forme(verbe, x, temps)))].filter(f => f !== bonne);
  const sonnePareil = memeTemps.filter(f => f.slice(0, -1) === bonne.slice(0, -1) || f.replace(/(s|t|ent)$/, '') === bonne.replace(/(s|t|ent)$/, ''));
  const soeur = (_sonnePareil$ = sonnePareil[0]) !== null && _sonnePareil$ !== void 0 ? _sonnePareil$ : memeTemps[0];
  const autresTemps = TEMPS.filter(t => t.cle !== temps).map(t => forme(verbe, pronom, t.cle)).filter(f => f !== bonne && f !== soeur);
 ...
 */
function formesProposees(verbe, pronom, tirer) {
...
function serie(graine = Date.now(), niveau = 'CE1') {
  ...
  for (let i = 0; i < MANCHES; i += 1) {
    let verbe;
    if (i === 2) verbe = 'être';else if (i === 6) verbe = 'avoir';else verbe = au(VERBES.slice(2));
    let pronom;
    do {
      pronom = au(PRONOMS);
    } while (liste.length > 0 && liste[liste.length - 1].pronom === pronom);
    liste.push({
      verbe,
      pronom,
      choix: formesProposees(verbe, pronom, tirer)
    });
  }
  return liste;
}
...
function serieCE2(tirer) {
  const au = l => l[Math.floor(tirer() * l.length)];
  const temps = ['present', 'imparfait', 'futur', 'imparfait', 'futur', 'present', 'imparfait', 'futur', 'present', 'futur'].sort(() => tirer() - 0.5);
  const liste = [];
  for (let i = 0; i < MANCHES; i += 1) {
    let verbe;
    if (i === 2) verbe = 'être';else if (i === 6) verbe = 'avoir';else verbe = au(VERBES.slice(2));
    ...
    liste.push({
      ...
      pronom,
      temps: temps[i],
      choix: formesCE2(verbe, pronom, temps[i], tirer)
    });
  }
  return liste;
}
...
function verdict(m, choix) {
  var _m$temps;
  const temps = (_m$temps = m.temps) !== null && _m$temps !== void 0 ? _m$temps : 'present';
