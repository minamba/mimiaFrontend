...
function serie(graine = Date.now()) {
  ...
  for (let i = 0; i < 3; i += 1) {
    ...
    liste.push({
      ...
      bonne,
      choix: melanger((0, _outils.troisChoix)(bonne, [ajout, total], k => euros(net(p * n2 + k / 2)))).map(c => ({
        cle: c,
        libelle: c
      })),
      pieges: {
  ...
  for (let i = 0; i < 3; i += 1) {
    ...
    liste.push({
      ...
      bonne,
      choix: melanger([bonne, oubli, virgule]).map(c => ({
        cle: c,
        libelle: c
      })),
      pieges: {
        [oubli]: 'vitesse-demi',
        [virgule]: 'vitesse-virgule'
      }
    });
  }
  for (let i = 0; i < 2; i += 1) {
    ...
    liste.push({
      ...
      bonne,
      choix: melanger((0, _outils.troisChoix)(bonne, [ecart], j => bonne + j * k)).map(c => ({
        cle: c,
        libelle: String(c)
      })),
      pieges: {
