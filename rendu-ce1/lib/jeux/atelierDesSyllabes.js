...
exports.SYLLABES_PARLANTES = exports.PHRASES = exports.MOTS = exports.MANCHES = exports.ESSAIS_AVANT_AIDE = exports.DEUX_SYLLABES = void 0;
exports.bilan = bilan;
exports.mot = mot;
exports.serie = serie;
exports.toutesLesSyllabes = toutesLesSyllabes;
...
function serie(graine = Date.now()) {
  ...
  return [...deux, ...trois].map(m => ({
    mot: m.cle,
    etiquettes: melanger([...m.syllabes, ...m.leurres]).map((texte, id) => ({
      id,
      texte
    }))
  }));
 ...
 */
function verdict(poses, cleMot) {
  const attendu = mot(cleMot).syllabes;
  if (poses.every((s, i) => s === attendu[i])) return {
    sens: 'juste',
    erreurs: []
  };
  ...
  poses.forEach((s, i) => {
    ...
    if (k === -1) erreurs.push(i);else restant.splice(k, 1);
  });
