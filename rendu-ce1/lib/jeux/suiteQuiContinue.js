...
exports.bilan = exports.PHRASES = exports.MANCHES = exports.FORMES = void 0;
exports.serie = serie;
...
function serie(graine = Date.now()) {
  ...
  for (let i = 0; i < 3; i += 1) {
    ...
    liste.push({
      ...
      bonne,
      choix: melanger((0, _outils.troisChoix)(bonne, [bonne + pas, termes[3] + 1], k => bonne + k)).map(c => ({
        cle: c,
        libelle: String(c)
      })),
      pieges: {
        [bonne + pas]: 'ajoute-saute',
        [termes[3] + 1]: 'ajoute-un'
      }
    });
  }
  for (let i = 0; i < 2; i += 1) {
    ...
    liste.push({
      ...
      question: `${termes.join(', ')}, … ?`,
      bonne,
      choix: melanger((0, _outils.troisChoix)(bonne, [ajout, termes[3] + termes[2]], k => bonne + k * f)).map(c => ({
        cle: c,
        libelle: String(c)
      })),
      pieges: {
        [ajout]: 'multiplie-ajout',
        [termes[3] + termes[2]]: 'multiplie-regle'
      }
    });
  }
  for (let i = 0; i < 3; i += 1) {
