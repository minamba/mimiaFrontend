...
exports.bilan = exports.PHRASES = exports.MANCHES = exports.AUXILIAIRES = exports.ACCORDS = void 0;
exports.serie = serie;
...
const ACCORDS = exports.ACCORDS = [['Les filles sont ___ au parc.', 'aller', 'allé', 'f', true], ['Mon frère est ___ de vélo.', 'tomber', 'tombé', 'm', false], ['Ma sœur est ___ très tôt.', 'partir', 'parti', 'f', false], ['Nos cousins sont ___ dîner.', 'venir', 'venu', 'm', true], ['Les feuilles sont ___ cette nuit.', 'tomber', 'tombé', 'f', true], ['Julie et Léa sont ___ en retard.', 'arriver', 'arrivé', 'f', true], ['Mes parents sont ___ tard.', 'rentrer', 'rentré', 'm', true], ['La neige est ___ toute la nuit.', 'tomber', 'tombé', 'f', false], ['Les oiseaux sont ___ vers le sud.', 'partir', 'parti', 'm', true]];

/** [phrase, bonne, piège de l'autre auxiliaire, piège de la personne]. */
const AUXILIAIRES = exports.AUXILIAIRES = [['Elle ___ partie hier.', 'est', 'a', 'sont'], ['Ils ___ venus à pied.', 'sont', 'ont', 'est'], ['Nous ___ mangé une pizza.', 'avons', 'sommes', 'ont'], ['Le bébé ___ tombé du lit.', 'est', 'a', 'sont'], ['Tu ___ fini tes devoirs.', 'as', 'es', 'a'], ['Les invités ___ arrivés à midi.', 'sont', 'ont', 'est']];
...
function serie(graine = Date.now()) {
  ...
  melanger(ACCORDS).slice(0, 5).forEach(([phrase, infinitif, p, genre, pluriel]) => {
    ...
    const pieges = {
      [sans]: 'accord-oubli',
      [autreGenre]: 'accord-genre',
      [autreNombre]: 'accord-nombre'
    };
    ...
    liste.push({
      ...
      bonne,
      choix: melanger([bonne, ...formes]).map(c => ({
        cle: c,
        libelle: c
      })),
      pieges
  ...
  melanger(AUXILIAIRES).slice(0, 3).forEach(([question, bonne, autre, personne]) => {
    liste.push({
      ...
      question,
      bonne,
      choix: melanger([bonne, autre, personne]).map(c => ({
        cle: c,
        libelle: c
      })),
      pieges: {
        [autre]: ['est', 'sont'].includes(bonne) ? 'auxiliaire-etre' : 'auxiliaire-avoir',
        [personne]: 'auxiliaire-personne'
      }
    });
  });
